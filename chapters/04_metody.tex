...
\begin{enumerate}
    \item Iterativní hledání globálních maximálních amplitud s použitím
          ...
          \begin{gather}
              R_i = max(W_i^{L,R}) \nonumber \\
              L = R = \frac{0.4}{2} Fs, ~L,R \in \mathbb{Z^+} \nonumber \\
...
R vlny a ektopický rytmus. Vizuálně lze artefakty identifikovat jako velké náhle
změny či kolísání v grafu jako je tomu na Obr. \ref{fig:hrv_artifacts} níže.
Podmínkou HRV analýzy je tedy použití N-N intervalů (normal-to-normal), které
představují korigované R-R intervaly. Korekcí je myšleno potlačení nežádoucích
artefaktů.
...
pohyblivých prahových hodnot.

V prvním kroku je vypočtena časová diference všech detekovaných R-R intervalů
$dRRs$, která slouží k rozlišení ektopických a špatně detekovaných R vln od
normálního sinusového rytmu \cite{Lipponen2019}:
...
jednotlivě chybějících srdečních stahů porovnáním $|dRR|>1$. Pro zbylé případy
falešné detekce R vln nebo jiných chyb, které nelze detekovat použitím $dRR$, je
definována série $mRRs$ jako rozdíl jednotlivých RR intervalů a mediánu 11
okolních hodnot:
