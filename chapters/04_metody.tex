...
Za účelem detekce R vln byl implementován a modifikován algoritmus podle
...
R vlny a ektopický rytmus, který společně s nesprávně detekovanými R vlny
...
Elipsa je konstruována využitím jejího vyjádření parametrickými rovnicemi:
\begin{gather}
...
Program byl vytvořen pomocí skriptovacího programovacího jazyka Python
\cite{python} s využitím frameworku PySide2 (viz sekce \ref{section:pyside}).
Hlavním účelem programu je online zpracovaní a vizualizace dat z měřícího
zařízení, aby bylo možné sledovat a hodnotit elektrickou srdeční aktivitu měřené
osoby v reálném čase. Aplikací lze data i zaznamenávat a vzápětí uložit ve
formátu CSV. K zajištěni minimální latence mezi vizualizací zpracovaných data a
přijatými aktuálními daty probíhá zpracování signálu, jeho analýza a záznam
paralelně (tzv. multithreading).
...
možné využívat aplikační rozhraní (API) knihovny \textit{Qt} v prostředí Python.
\textit{Qt} není pouze knihovna ale celý nativní multiplatformní framework pro
vytváření softwaru s GUI, napsaný v programovacím jazyce \textit{C++}. V
aplikaci \textit{BBPM} byla použita knihovna \textit{PySide2}, která představuje
port pro \textit{Qt5}.
...
\begin{figure}[h]
    \begin{center}
        \textcolor{black}{\fboxrule=0.5pt\fboxsep=0pt\fbox{\includegraphics[width=1\textwidth]{../assets/bbpm/qt_designer}}}
...
zobrazuje karta podle zvolené položky v menu.

Vysouvací menu obsahuje čtyři tlačítka, přičemž první tlačítko, které se nachází
v levém horním rohu hlavního okna, iniciuje vysunutí a zasunutí menu. Jednotlivé
funkce ostatních tlačítek jsou:
\begin{itemize}[noitemsep]
...
\begin{figure}[h]
    \begin{center}
        \textcolor{cyan}{\fboxrule=0.5pt\fboxsep=0pt\fbox{\includegraphics[width=1\textwidth]{../assets/bbpm/main_window}}}
...
Poslední informační subpanel (Events) 

\begin{figure}[h]
    \centering
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \textcolor{cyan}{\fboxrule=0.5pt\fboxsep=0pt\fbox{\includegraphics[width=1\linewidth]{../assets/bbpm/settings_general}}}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \textcolor{cyan}{\fboxrule=0.5pt\fboxsep=0pt\fbox{\includegraphics[width=1\linewidth]{../assets/bbpm/settings_graph}}}
    ...
    \begin{subfigure}[b]{0.3\textwidth}
        \centering
        \textcolor{cyan}{\fboxrule=0.5pt\fboxsep=0pt\fbox{\includegraphics[width=1\linewidth]{../assets/bbpm/settings_recording}}}
